...
########## one layer model with sigmoid activation function
...
def base_power_measurement():
    ...
    with open("results/basic_layers/sigmoid/idle_powerstat_bench.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    mem_file = open(f"results/basic_layers/sigmoid/powerstat_bench.txt", "w")
    ...
    file = open("results/basic_layers/sigmoid/data.csv", "a", newline='')
    ...
    for i in range(len(vector_sizes)):
        writer.writerow([vector_sizes[i], round(plain_times[i],5), round(plain_times_std[i],5), round(cipher_times[i],5), round(cipher_times_std[i],5), round(mean_error[i],5), sample_sizes[i]-warmup])
        file.flush()
